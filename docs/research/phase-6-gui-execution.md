...
| D45 (PR-C) | Profile-selector source of truth | Read `tethys_profile.h` markers via the slave's `GET_VERSION` response; mirror to `tethys_master.config.MasterSettings.profile` for offline review | Compile-time only (loses runtime switching for marine ↔ space dev demos) | (to be filled at PR-C) |
